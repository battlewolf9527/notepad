...
async function generateLink() {
  const url = await shareStore.generateRawLink(
    props.filename,
  ...
  <div v-if="visible" class="modal-overlay" @click.self="emit('close')">
    ...
    >
      ...
      <div class="dialog-body">
        ...
        <div class="form-group">
          <label>CORS 来源（默认允许所有）:</label>
          <input v-model="corsOrigin" type="text" placeholder="* 或指定域名" class="input" />
        </div>
...
<style scoped>
.raw-dialog {
  min-width: calc(380px * var(--scale));
}
